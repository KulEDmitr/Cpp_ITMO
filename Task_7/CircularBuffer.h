...
public:
    CircularBuffer(const CircularBuffer<T> &other, int cap = -1) :
            capacity((cap == -1) ? other.capacity : cap), size(0) {
    ...
    void checkSize() {
        if (empty()) {
            throw std::runtime_error("the last guy was the least, sorry!");
        }
    }

    friend std::ostream &operator<<(std::ostream &s, CircularBuffer<T> &a) {
        s << "Hello, I'm a circular buffer (maximal size is " << a.capacity << ") with " << a.size << " elements:\n[";
        int i = 0;
